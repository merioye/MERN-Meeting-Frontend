...
const TransitionComponent = React.forwardRef(function TransitionComponent(
    props,
    ref
) {
    ...
    const shareFile = async (e) => {
        ...
        try {
            ...
            const res = await fetch(
                `${process.env.REACT_APP_API_BASE_URL}/rooms/${roomId}/uploadfile`,
                {
                    credentials: "include",
                    method: "POST",
                    body: formData,
            ...
            console.log(e);
            dispatch(
                setErrorAlert("File not shared due to some problem, try again")
            );
        }
    };
            ...
            <Box sx={style.share}>
                <Box sx={style.shareItems}>
                    <label htmlFor="photo">
                        ...
                        <Button
                            component="span"
                            sx={style.shareItemBtn}
                            style={{ borderTop: "1px solid #f2f2f217" }}
                        >
                            Photo
                            <PhotoSizeSelectActualOutlinedIcon
                                sx={style.shareItemBtnIcon}
                            />
                        </Button>
                    </label>
